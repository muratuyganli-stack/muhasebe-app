...
# Veritabanı v21 - Aynı isimle birden fazla kayıt desteği eklendi
def init_db():
    conn = sqlite3.connect('muhasebe_v21.db', check_same_thread=False)
    c = conn.cursor()
    # 'ad TEXT UNIQUE' yerine 'ad TEXT' yapılarak isim kısıtlaması kaldırıldı
...
if st.session_state.get('yeni_m'):
    with st.form("m_form"):
        ...
        if c1.form_submit_button("✅ KAYDET"):
            if y_ad:
                # Aynı isimli müşteriye izin veriliyor
    ...
    for _, m in df_m.iterrows():
        if arama.lower() in m['ad'].lower():
            ...
            with st.container(border=True):
                ...
                with col1:
                    # Müşteri adının yanına telefon ekleyerek ayrımı kolaylaştırıyoruz
                    st.markdown(f"**{m['ad']}**")
                    if m['tel']: st.caption(f"📞 {m['tel']}")
                with col2:
...
# Detay ve Fotoğraf kısmı v20 ile aynı (Fotoğraflar aynen korunuyor)
